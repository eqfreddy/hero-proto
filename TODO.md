...
Last updated: 2026-05-11 (Battle 3D v1.1 layout pass — archetype lookup via participants, SkeletonUtils.clone fix for shared-skeleton stacking, formation stagger, HUD portraits, diorama auto-fit + push-back. Responsive sizing, action buttons, and weapons back-burnered.)
...
- [x] ~~**Replace Throw fallback with procedural arm-swing**~~ ✅ Shipped 2026-05-10 PM. `frontend/src/battle3d/proceduralClips.ts` builds a 400ms diagonal swing clip per-skeleton at hero load via pattern-matched right-arm bone discovery (upper/fore/hand by `right|_r` heuristics). `clipMap.ts` prefers `MeleeSwing` over `Throw`; null-on-discovery-failure silently falls back so missing bones don't regress. 6 new procedural tests + 2 heroLoader prepend tests. **Live verification still pending** — if KayKit bone names don't match the regex heuristics, the clip will silently no-op and `Throw` resumes. Spec: `docs/superpowers/specs/2026-05-10-battle-3d-procedural-melee-swing.md`.
- [ ] **Monster select + team-builder UX overhaul** — flagged 2026-05-10 PM during live verification. Current team-select / battle-setup flow is rough enough that the user called it out unprompted. Needs a proper design pass: hero filtering by archetype/rarity, drag-to-slot, faction synergy preview, save/load loadouts. Likely `frontend/src/routes/battle/BattleSetupRoute.tsx` + a new TeamBuilder component. Scope it before plan.
- [ ] **Interactive battle: target-pick clicks don't register in 3D mode** — reported 2026-05-10 PM. User saw "pick a target" prompt over the Three.js scene but clicking the canvas (3D dragon models) did nothing. Likely cause: there's no raycast handler on `Battle3DScene` — only the HUD UnitCards in `BattleHUD.tsx` are clickable. Either (a) document that the cards at the top of the screen are the click target, or (b) add a raycaster to the canvas that maps clicked rig → unit UID → `act(uid)`. Option (b) is the better UX and the obvious next 3D feature.
